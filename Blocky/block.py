...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        for i in range(len(self.children)):
            self.children[i].position = pos_lst[i]
    ...
    def smash(self) -> bool:
        ...
        for i in range(len(child_pos)):
            self.children.append(Block(child_pos[i], int(self.size / 2),
                                       random.choice(COLOUR_LIST),
                                       self.level + 1,
                                       self.max_depth))
            # Potentially smashing children
            num = random.random()
            if num < math.exp(-0.25 * (self.level + 1)) and \
                    self.children[i].smashable():
                self.children[i].smash()
            else:
                self.children[i].colour = random.choice(COLOUR_LIST)
        self.colour = None
        return True
    ...
    def rotate(self, direction: int) -> bool:
        ...
        if len(self.children) == 0:
            ...
            self.children = [self.children[-1]] + self.children[:3]
            for child in self.children:
                child.rotate(direction)
        self._update_children_positions(self.position)
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if self.children or self.level != self.max_depth or \
                self.colour == colour:
            return False
        self.colour = colour
        return True
    ...
    def combine(self) -> bool:
        ...
        if self.level != self.max_depth - 1 or self.colour is not None:
            return False
        colours = [x.colour for x in self.children]
        maj_clr = None
        max_ = 0
        for item in colours:
            x = colours.count(item)
            if x > 2:
                maj_clr = item
                break
            elif x == max_ and item != maj_clr:
                maj_clr = None
                break
            elif x > max_:
                max_ = x
    ...
    def create_copy(self) -> Block:
        ...
        copy = Block(self.position, self.size, self.colour, self.level,
                     self.max_depth)
        if copy.colour is None:
            copy.children = [x.create_copy() for x in self.children]
        return copy
