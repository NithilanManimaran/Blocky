...
def create_players(num_human: int, num_random: int, smart_players: List[int]) \
    ...
    for i in range(len(goals)):
        if i < num_human:
            ...
            lst.append(SmartPlayer(i, goals[i],
...
def _get_block(block: Block, location: Tuple[int, int], level: int) -> \
    ...
    if block.level == level:
        x, y = block.position
        if x <= location[0] < x + block.size and y <= location[1] \
                < y + block.size:
            return block
        else:
            return None
    if block.colour is not None:
        ...
        for item in block.children:
            x = _get_block(item, location, level)
            if x is not None:
                return x
    return None
...
class RandomPlayer(Player):
    ...
    def generate_move(self, board: Block) -> \
        ...
        action_lst = [SMASH, ROTATE_CLOCKWISE, ROTATE_COUNTER_CLOCKWISE,
                      PAINT, COMBINE, SWAP_HORIZONTAL, SWAP_VERTICAL]
        # Get a random block
        rand_block, test = self._get_random_block(board, copy)
        # Get a random action
        while action_lst:
            action = random.choice(action_lst)
            if action == SMASH:
                act = test.smash()
                if act:
                    self._proceed = False
                    return SMASH + (rand_block,)
            elif action == ROTATE_CLOCKWISE:
                act = test.rotate(1)
                if act:
                    self._proceed = False
                    return ROTATE_CLOCKWISE + (rand_block,)
            elif action == ROTATE_COUNTER_CLOCKWISE:
                act = test.rotate(3)
                if act:
                    self._proceed = False
                    return ROTATE_COUNTER_CLOCKWISE + (rand_block,)
            elif action == PAINT:
                act = test.paint(self.goal.colour)
                if act:
                    self._proceed = False
                    return PAINT + (rand_block,)
            elif action == COMBINE:
                act = test.combine()
                if act:
                    self._proceed = False
                    return COMBINE + (rand_block,)
            elif action == SWAP_VERTICAL:
                act = test.swap(1)
                if act:
                    self._proceed = False
                    return SWAP_VERTICAL + (rand_block,)
            elif action == SWAP_HORIZONTAL:
                act = test.swap(0)
                if act:
                    self._proceed = False
                    return SWAP_HORIZONTAL + (rand_block,)
            else:
                action_lst.remove(action)
        self._proceed = False
        return None

    # Helper
    def _get_random_block(self, board: Block, copy: Block) -> \
            Tuple[Block, Block]:
        """Return a random block at a random depth from <board> and a copy of
        that same block from <copy>.

        Precondition: <copy> is a deep copy of <board>
        """
        if board.level == board.max_depth or len(board.children) == 0:
            return board, copy
        else:
            r = random.randint(0, 3)
            num = random.randint(0, 1)
            if num == 0:
                return self._get_random_block(board.children[r],
                                              copy.children[r])
            else:
                return board.children[r], copy.children[r]
